...
if (session == null || session.getAttribute("email") == null) {
    ...
    return;
        ...
        try
        {
            ...
            String job = req.getParameter("u_job");
            String salary = req.getParameter("u_salary");
            ...
            PreparedStatement psmt = con.prepareStatement("update employee set name = ? , email = ? , mobile = ? , gender = ? , job = ? , salary =  ? , age = ? , password = ? where adhar = ? ");
            ...
            psmt.setString(6, salary);
            psmt.setString(7, age);
            psmt.setString(8, pwd);
            psmt.setString(9, adhar);
            
            boolean b = psmt.execute();
            if (b == false)
            {
                resp.setContentType("text/html");
                out.println("<h2 style='color: green; text-align: center;'><b>Your Employee Detail's Updated Succesfully</b></h2>");
